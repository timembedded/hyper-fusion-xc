...
FM_OPL *OPLCreate(int type, int clock, int rate, int sampleram, void* ref)
{
    ...
    if(type&OPL_TYPE_ADPCM) {
        ...
        OPL->deltat->memory = heap_caps_malloc(sampleram*1024*sizeof(UINT8), MALLOC_CAP_SPIRAM); /* size of sample bank */
...
{
    OPL_UnLockTable();
    heap_caps_free(OPL->deltat->memory);
    free(OPL);
}
