...
private:
    ...
    // Tables generated at creation, will be allocated in PSRAM
    struct tables_t {
        // dB to linear table (used by Slot)
        short dB2LinTab[(DB_MUTE + DB_MUTE) * 2];

        // WaveTable for each envelope amp
        word fullsintable[PG_WIDTH];
        word halfsintable[PG_WIDTH];
        word* waveform[2];

        unsigned int dphaseNoiseTable[512][8];

        // LFO Table
        int pmtable[PM_PG_WIDTH];
        int amtable[AM_PG_WIDTH];

        // Liner to Log curve conversion table (for Attack rate).
        word AR_ADJUST_TABLE[1 << EG_BITS];
        ...
        // KSL + TL Table
        unsigned int tllTable[16][8][1 << TL_BITS][4];
        int rksTable[2][8][2];
